...
const Cart = () => {
    const {products, currency, cartItems, removeFromCart, getCartCount, updateCartItem, navigate, getCartAmount, user, setIsLoggedIn} = useAppContext()
   ...
   const placeOrder = async () => {
    ...
    if(!user){
        ...
        setIsLoggedIn(false); // login modal open
        return;
    }
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                            ...
                            <div>
                                ...
                                <div className="font-normal text-gray-500/70">
                                    ...
                                    <div className='flex items-center'>
                                        <p>Qty:</p>
                                        <select value={cartItems[product._id]} className='outline-none' onChange={e => updateCartItem(product._id, Number(e.target.value))}>
                        ...
                        <button onClick={() => removeFromCart(product._id)} className="cursor-pointer mx-auto">
                           <img src={assets.refresh_icon} alt="Refresh" className='inline-block w-6 h-6'/>
                        </button>
                    </div>)
                )}

                <button onClick={() => { navigate('/products'); scrollTo(0, 0); }} className="group cursor-pointer flex items-center mt-8 gap-2 text-primary font-medium">
                    <img src={assets.arrow_right_icon_colored} alt="Continue Shopping"/>
                    Continue Shopping
                </button>
