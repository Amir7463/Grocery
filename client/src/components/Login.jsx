...
const Login = () => {
    ...
    const {setIsLoggedIn, user, setUser, setShowLogin,axios} = useAppContext();
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
        if (state === "register") {
            ...
            if (data.success) {
                ...
                if (!userObj._id && data.user._id) userObj._id = data.user._id;
                setUser(userObj);
                setIsLoggedIn(true);
    ...
    return (
        <div onClick={() => setShowLogin(false)} className="fixed top-0 bottom-0 left-0 right-0 z-30 flex items-center text-sm text-gray-600 bg-black/50">
