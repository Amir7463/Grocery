...
const ProductCard = ({ product }) => {
  ...
  return (
    <div 
      ...
      className="border border-gray-500/20 rounded-md md:px-4 px-3 py-2 bg-white w-full cursor-pointer"
    >
      <div className="group flex items-center justify-center px-2">
        <img 
          className="group-hover:scale-105 transition max-w-26 md:max-w-36" 
          src={product.images?.[0] || assets.fallback_image} 
          alt={product.name} 
        />
      </div>
          ...
          <div onClick={(e) => e.stopPropagation()}>
            {!cartItems[product._id] ? (
              ...
              <div className="flex items-center justify-center gap-2 md:w-20 w-16 h-[34px] bg-primary/25 rounded select-none">
                <button onClick={() => removeFromCart(product._id)} className="cursor-pointer px-2">-</button>
                <span className="w-5 text-center">{cartItems[product._id]}</span>
                <button onClick={() => addToCart(product._id)} className="cursor-pointer px-2">+</button>
              </div>
            )}
          </div>
        </div>
